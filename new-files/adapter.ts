...
//
// Translates between the app's existing types (Task, CalendarEvent,
// GmailSuggestion — defined in @/types/planner and GoogleContext) and the
...
// framework/app-agnostic and the rest of PlannerContext stays untouched.
...
import type { CalendarEvent } from "@/contexts/GoogleContext";
...
export function focusSessionsToHistory(
  ...
  // Roll up the last 7 known days into planned-vs-completed minutes. Falls
  // back to a neutral entry if there's not enough data — RecoveryEngine
  // treats history.length < 3 as "no real signal" anyway.
...
export function buildEngineContext(opts: {
  ...
  const busy = opts.calendarEvents
